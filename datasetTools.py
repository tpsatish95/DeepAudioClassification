...
def loadDataset(nbPerGenre, genres, sliceSize, mode):
    ...
    if mode == "train":
        ...
        validation_X = pickle.load(
            open("{}validation_X_{}.p".format(datasetPath, datasetName), "rb"))
        validation_y = pickle.load(
            open("{}validation_y_{}.p".format(datasetPath, datasetName), "rb"))
...
def createDatasetFromSlices(nbPerGenre, genres, sliceSize, validationRatio, testRatio):
    ...
    validation_X = np.array(X[trainNb:trainNb + validationNb]
                            ).reshape([-1, sliceSize, sliceSize, 1])
    ...
    # Save
    saveDataset(train_X, train_y, validation_X, validation_y,
                test_X, test_y, nbPerGenre, genres, sliceSize)
